  ...
  try
    ...
    begin
      ...
      begin
        ...
        if password <> '' then
        begin
          Writeln(GetDosOutput(Format('%s\mysqldump.exe -u %s -p %s --databases %s > %sbackup_%d.sql', [mysqlDir, user, password, database, dirToSave, DateTimeToUNIXTimeFAST(timestamp)])));
        end
        else
        begin
          Writeln(GetDosOutput(Format('%s\mysqldump.exe -u %s --databases %s > %sbackup_%d.sql', [mysqlDir, user, database, dirToSave, DateTimeToUNIXTimeFAST(timestamp)])));
        end;
